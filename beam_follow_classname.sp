...
// Plugin Info - define
// ====================================================================================================
#define PLUGIN_NAME                   "[ANY] Beam Follow by Classname"
...
void LoadConfigs()
{
    ...
    if (kv.JumpToKey("default"))
    {
        ...
        if (defaultConfig.model[0] != 0)
            defaultConfig.modelIndex = PrecacheModel(defaultConfig.model);
    ...
    if (kv.JumpToKey("classnames"))
    {
        if (kv.GotoFirstSubKey())
        {
            do
            {
                ...
                BeamSetup config;
                config.enable = (kv.GetNum("enable", defaultConfig.enable) == 1);
                kv.GetString("model", config.model, sizeof(config.model), defaultConfig.model);
                if (config.model[0] != 0)
                    config.modelIndex = PrecacheModel(config.model);
