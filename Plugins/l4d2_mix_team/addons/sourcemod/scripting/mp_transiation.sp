...
char sMapStart[64], sMapEnd[64];

#include "include/mp_transiation.inc"
...
{
	name = "Map Transitions for MixMap",
	author = "Derpduck, Forgetest, P",
...
{
    CreateNative("AddMapTransition", Native_AddMapTransition);
    CreateNative("ResetAllMapTransition", Native_ResetAllMapTransition);
	CreateNative("RemoveMapTransition", Native_RemoveMapTransition);
	RegPluginLibrary("mp_transiation");
...
public int Native_ResetAllMapTransition(Handle plugin, int numParams){
    g_hMapTransitionPair.Clear();
	return 0;
}

public int Native_RemoveMapTransition(Handle plugin, int numParams){
	GetNativeString(1, sMapStart, sizeof(sMapStart));
	bool res = g_hMapTransitionPair.Remove(sMapStart);
	if (res){
		return 0;
	}else{
		return 1;
