...
#define SECRET_SOUND "ui/alert_clink.wav"
#define DEFAULT_COUNTDOWN_SOUND "weapons/50cal/50cal_shoot.wav"
#define DEFAULT_LIVE_SOUND "level/loud/bell_break.wav"
...
{
	sp_hostname                 = CreateConVar("sp_hostname", "Rumbling #2", "Fuck bug", FCVAR_NOTIFY);
...
void PrintCmd()
{
	int iSurvivorLimit = survivor_limit.IntValue;
    int maxHealthBonu = SMPlus_GetMaxHealthBonus();
    int maxDamageBonus	= SMPlus_GetMaxDamageBonus();
	...
	{
        iScoreDifference = 0 - iScoreDifference;
	}
...
void UpdatePanel()
{
	...
	if (bufLen != 0)
	{
		...
		Format(nameBuf, sizeof(nameBuf), "->%d. Surs♥ - %d", ++textCount, L4D2Direct_GetVSCampaignScore(L4D2_AreTeamsFlipped()));
	...
	if (bufLen != 0)
	{
		...
		Format(nameBuf, sizeof(nameBuf), "->%d. Inf♥ - %d", ++textCount, L4D2Direct_GetVSCampaignScore(!L4D2_AreTeamsFlipped()));
		menuPanel.DrawText(nameBuf);
	...
	{
		specBuffer[bufLen] = '\0';
		Format(nameBuf, sizeof(nameBuf), "->%d. Spec%s♥", ++textCount, specCount > 1 ? "s" : "");
		menuPanel.DrawText(nameBuf);
